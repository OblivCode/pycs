...
namespace pycs.modules
{
    public class logging
    {
        ...
        //static functions
        public static logging basicConfig(string filename = null, string format = null, severity level = severity.none) => new logging(new StreamReader(filename).BaseStream, format: format, level: level);
        ...
        public static logging FileHandler(string filename) => new logging(new StreamReader(filename).BaseStream);
        ...
        {
            if (stream == null)
                throw new NullReferenceException("Stream not assigned");
            stream.Flush();
        }
        /// <exception cref="Exception"></exception>
        public void close()
        ...
        private void log_print(string msg, string override_level = null)
        {
            ...
            if (stream != null)
                ...
                catch(Exception e) { throw new Exception("Failed to write to stream", e); }
            else
